...
// A perfect file behaves like a perfect file.
// It reads/writes Instantly (for large values of instant)
// It does not error (system memory permitting)
// It supports concurrent access (for appropriate methods)
type PerfectFile struct {
...
func (pf *PerfectFile) Grow(desired_size int64) {
	pf.Lock()
	defer pf.Unlock()
...
func (pf *PerfectFile) Seek(offset int64, whence int) (int64, error) {
	pf.Lock()
	defer pf.Unlock()
...
func (pf *PerfectFile) writeAtWk(p []byte, offset int64) (n int, err error) {
...
func (pf *PerfectFile) WriteAt(p []byte, offset int64) (n int, err error) {
	pf.Lock()
	defer pf.Unlock()
...
func (pf *PerfectFile) Write(p []byte) (n int, err error) {
	pf.Lock()
...
func (pf *PerfectFile) ReadAt(p []byte, offset int64) (n int, err error) {
	pf.Lock()
	if pf.store == nil {
		pf.Unlock()
		return 0, io.EOF
...
func (pf *PerfectFile) Read(p []byte) (n int, err error) {
	if pf.store == nil {
		return 0, io.EOF
	}
	pf.Lock()
...
func (pf *PerfectFile) Cap() int {
	return cap(pf.store)
}
func (pf *PerfectFile) Len() int {
	return len(pf.store)
}
